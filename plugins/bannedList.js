...
╰────
`.trim()
    conn.send2ButtonImg(m.chat, await(await fetch(thumbfoto)).buffer(), caption, watermark, `Menu`, `${usedPrefix}menu`, `Owner`, `.owner`, m, { contextInfo: { mentionedJid: conn.parseMention(caption) }, mentions: await conn.parseMention(caption) })
}
